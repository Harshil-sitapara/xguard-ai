...
2. `Prediction` - 16 edges
3. `TokenScope` - 15 edges
4. `TrafficReplayManager` - 14 edges
5. `Base` - 9 edges
6. `Alert` - 9 edges
7. `POST /api/v1/predict  — single and batch prediction endpoints.` - 8 edges
8. `Classify a single network flow and return label + confidence.` - 8 edges
9. `Classify up to 1000 network flows in one request.` - 8 edges
10. `process_traffic_message()` - 8 edges
...
- `XGuard-AI FastAPI application.  Lifespan:   startup -> load model artefacts,` --uses--> `Base`  [INFERRED]
...
- **Are the 11 inferred relationships involving `TokenScope` (e.g. with `Alerts: REST history + WebSocket live stream.` and `Paginated alert history with optional attack_type filter.`) actually correct?**
  _`TokenScope` has 11 INFERRED edges - model-reasoned connections that need verification._
